...
Source document retained and reorganized from: docs/research/20_SIMULATION.md
...
Before hardware integration, simulate users, merchants, connectivity failures, fraud attempts, and reconciliation.
...
- 1,000 synthetic merchants;
- configurable transaction arrival rate;
- configurable connectivity states C0–C3;
- configurable fraud strategy.

These are experiment parameters, not real-world forecasts.
...
- reconciliation delay distribution;
- conflict rate;
- economic exposure under attack;
- transport-specific availability;
- model performance.

## 6. Determinism

Every stochastic run must support a fixed random seed for reproduction.
