...
Source document retained and reorganized from: docs/development/INTERFACES.md
...
```kotlin
interface Signer {
    suspend fun sign(canonicalPayload: ByteArray): Signature
    suspend fun publicKey(): PublicKey
}
```

The actual implementation may delegate to Rust and/or Android Keystore-backed operations.

## Reconciliation client

```kotlin
interface ReconciliationClient {
    suspend fun submit(batch: List<SignedPaymentEnvelope>): ReconciliationResult
}
```

Interfaces are contracts. Do not add cross-layer shortcuts to avoid implementing the intended boundary.
