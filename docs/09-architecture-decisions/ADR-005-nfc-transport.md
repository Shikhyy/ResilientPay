...
Source document retained and reorganized from: docs/architecture/adr/ADR-005-nfc-transport.md
Authority: Architecture Decision Record
This file is normative unless explicitly marked as informative in its body.
-->
# ADR-005 - NFC Transport
...
## Decision
Implement NFC as one transport adapter, not as a separate payment engine.

## Rationale
NFC is relevant to existing offline UPI-adjacent capabilities and supports direct proximity exchange on compatible devices.
...
The first NFC experiment should minimize assumptions: exchange a bounded protocol payload, verify it, persist the result, and measure time/size. Do not begin by tying payment authorization directly to Android NFC callbacks. The callback only reports the result of the transport operation.

## Security model

Physical proximity reduces the communication range but does not prove who is on the other side. The received protocol payload still requires signature/credential verification. NFC tags/QRs can be copied, so merchant identity must be authenticated at the protocol layer.

## Device matrix

Record Android API level, manufacturer/model, NFC capabilities, screen state constraints, and observed failure modes for every real-device experiment.
