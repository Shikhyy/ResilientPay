...
Source document retained and reorganized from: docs/architecture/adr/ADR-003-go-backend.md
...
## Decision
Use Go for the prototype backend.

## Rationale
Simple deployment model, strong standard library, explicit concurrency model, and suitability for API/reconciliation services.
...
Prefer a small number of explicit packages: `api`, `protocol`, `reconciliation`, `credential`, `ledger`, `risk`, and `observability`. HTTP handlers should validate transport-level request shape and authentication, then call domain services. SQL should not be embedded throughout handlers.

## Concurrency

The backend must assume concurrent retries. A request may arrive while another worker is processing the same transaction. Use database uniqueness and transaction boundaries to make duplicate processing safe rather than relying solely on in-memory locks. Timeouts and cancellation must propagate through all network/database calls.

## Research consequence

Go's simplicity is useful here because the reconciliation service is an explicit state machine plus persistence boundary. Complexity should be added only when a measured experiment requires it.
