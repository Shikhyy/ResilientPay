...
Source document retained and reorganized from: docs/architecture/adr/ADR-007-no-blockchain-core.md
Authority: Architecture Decision Record
This file is normative unless explicitly marked as informative in its body.
-->
# ADR-007 - No Blockchain in the Core Payment Path
...
## Decision
Do not use blockchain as the authoritative payment-path ledger.

## Rationale
The prototype has a trusted reconciliation authority. A centralized transactional database plus append-only/hash-linked evidence is simpler, faster, and easier to control.

## Research exception
A blockchain audit/reconciliation variant may be built as a comparison experiment. It must not silently become the core design.
...
A disconnected device cannot consult a global blockchain consensus state at the point of payment unless it has an alternative pre-authorized mechanism. That means blockchain does not remove the central problem of stale information. The core research should therefore isolate what cryptographic signatures, counters, bounded budgets, and later reconciliation accomplish without introducing distributed consensus.

## Optional comparison experiment

A later experiment may implement a ledger hash or blockchain-style append-only structure and compare: write latency, confirmation latency, storage growth, node requirements, fault assumptions, auditability, and operational complexity. The comparison must use the same evidence workload and clearly state the trust model.
