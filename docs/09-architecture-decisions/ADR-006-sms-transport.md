...
Source document retained and reorganized from: docs/architecture/adr/ADR-006-sms-transport.md
Authority: Architecture Decision Record
This file is normative unless explicitly marked as informative in its body.
-->
# ADR-006 - SMS Store-and-Forward
...
## Decision
Use SMS as an authenticated store-and-forward transport when telecom connectivity exists.

## Rationale
It provides a resilience path distinct from Internet access, while remaining outside the strict RBI definition of an offline payment because telecom connectivity is required.
...
The SMS implementation must behave like an asynchronous queue, not an RPC. A message can be delayed, duplicated, or never arrive. The sender therefore persists the message before attempting delivery and keeps it pending until a higher-level acknowledgment or reconciliation result is available.

## Fragmentation

If a protocol payload exceeds the practical SMS size, fragment using explicit message ID, part number, part count, and integrity-protected content. Reassembly must reject inconsistent fragments and must expire incomplete assemblies. Do not build an unbounded memory queue from unauthenticated message counts.

## Operational note

Android permissions, carrier behavior, provider APIs, and message-size rules are environment-specific. The prototype must isolate those details in the transport adapter and record them in the experiment environment.
