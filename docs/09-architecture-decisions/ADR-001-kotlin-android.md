...
Source document retained and reorganized from: docs/architecture/adr/ADR-001-kotlin-android.md
Authority: Architecture Decision Record
This file is normative unless explicitly marked as informative in its body.
-->
# ADR-001 - Kotlin Native Android
...
## Decision
Use Kotlin and native Android APIs for payer and merchant prototypes.

## Rationale
NFC, BLE, Keystore, lifecycle, background behavior, and device-specific capabilities are first-class concerns. Native APIs reduce abstraction leakage in these security/transport paths.

## Consequence
UI/product reuse across platforms is not optimized; protocol/core logic stays platform-neutral to reduce lock-in.
...
<!-- Deepened -->
## Implementation consequences in detail

The Android codebase should be divided into `presentation`, `application/domain`, `data`, `platform`, and `protocol` areas. The domain layer must be runnable in JVM unit tests without requiring a physical radio, Activity, or Android UI. Android APIs should enter through small adapters. This structure lets the same authorization/state tests run in CI without devices.

Keystore integration is especially important: the application should request cryptographic operations through a capability interface. It should not read raw key bytes simply because a test implementation can. A fake signer may be used in unit tests, but the fake must live behind the same interface and be clearly scoped to test builds.

## Acceptance tests

- A release build contains no debug bypass that replaces the secure signer.
- Domain tests execute without Android UI components.
- A process restart preserves the local counter/budget state.
- Unsupported device capabilities produce explicit transport errors instead of silent fallback.
