...
Source document retained and reorganized from: docs/development/CONFIGURATION.md
...
SMS_PROVIDER=mock
RISK_MODEL_VERSION=baseline-v1
OFFLINE_POLICY_VERSION=research-v1
```

## Secret handling

Secrets belong in the runtime secret mechanism of the environment, not source control or `.env` files committed to the repository.

## Policy configuration

Offline limits, credential expiry, retry windows, and risk thresholds must be explicit versioned configuration. Avoid scattered hard-coded constants.

## Environment rule

Development, test, staging, and production-like settings must be distinguishable. The research prototype may not point at unknown external payment infrastructure.
