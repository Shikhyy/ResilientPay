...
Source document retained and reorganized from: docs/development/DEPENDENCIES.md
...
- pinned version or controlled version range;
- purpose;
- license;
...
Crypto, serialization, networking, storage, and authentication libraries require extra review.

## Agent rule

Agents must not add a dependency merely to save a few lines of code when the functionality can be implemented safely with existing project dependencies or the platform.

## Update policy

Dependency upgrades must include changelog/security review and relevant regression tests.
