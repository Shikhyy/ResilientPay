...
Source document retained and reorganized from: docs/development/OBSERVABILITY.md
...
This document complements `.agent/OBSERVABILITY.md` with implementation guidance.

## Correlation

Every request and transaction path should support a correlation ID. Backend-generated IDs are preferable for backend events.

## Redaction

Sensitive protocol fields must be classified before logging. A debugging view may display hashes or truncated identifiers rather than raw secret material.

## Reliability signals

Track transport-level failures separately from protocol/security failures so operators/researchers can distinguish connectivity problems from malicious or malformed traffic.
