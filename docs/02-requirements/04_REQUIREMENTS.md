<!--
ResilientPay professional documentation package.
Source document retained and reorganized from: docs/research/04_REQUIREMENTS.md
Authority: Requirements specification
This file is normative unless explicitly marked as informative in its body.
-->
# 04 - Requirements

> **Document role:** Normative source-of-truth document.

## 1. Requirement notation

- `FR-*` functional requirement.
- `NFR-*` non-functional requirement.
- `SEC-*` security requirement.
- `RES-*` resilience requirement.
- `RESR-*` research requirement.

## 2. Functional requirements

### FR-001 - Create transaction
The payer application shall create a transaction object containing only fields defined by the protocol schema.

### FR-002 - Authenticate transaction
The transaction shall be cryptographically authenticated before it is transferred as an authorized offline payment.

### FR-003 - Verify transaction
The merchant shall verify the signature, credential status, counter/replay rule, value policy, and protocol version before accepting the transaction locally.

### FR-004 - Persist evidence
Both sides shall persist enough transaction/event data to reconstruct the local state and support reconciliation.

### FR-005 - Multi-modal transfer
The same logical transaction shall be transferable via supported transport adapters without changing business semantics.

### FR-006 - Reconcile
The backend shall accept authenticated transaction evidence, apply idempotent processing, record conflicts, and return a deterministic reconciliation result.

### FR-007 - Risk score
The risk engine may produce a risk score, but the payment protocol shall remain safe if the model is unavailable.

## 3. Non-functional requirements

- Deterministic state transitions.
- Integer monetary representation.
- Idempotent backend ingestion.
- Secure key storage.
- Testable module boundaries.
- Reproducible experiments.
- Versioned protocol schemas.

## 4. Security requirements

- Private signing keys must remain non-exportable where platform support permits.
- Signature input must have canonical serialization.
- Replay protection cannot rely on timestamps alone.
- Secret material must never be written to ordinary logs.
- Client-provided final states must not be blindly trusted by the backend.
- Invalid signatures fail closed.
- Cryptographic algorithms must come from approved libraries/primitives.

## 5. Resilience requirements

- Transport failure must not corrupt payment state.
- Duplicate messages must be safe to process repeatedly.
- Reordered messages must be handled explicitly.
- Lost messages must be retryable where safe.
- Reconciliation must be resumable after process/device restart.

## 6. Research requirements

Every major design choice must map to an experiment or be labeled as an engineering assumption.

<!-- Enriched: detailed implementation guidance -->


## Engineering interpretation and implementation notes

### Normative language
The words **MUST**, **MUST NOT**, **SHOULD**, **SHOULD NOT**, and **MAY** are used intentionally. MUST/MUST NOT define requirements that an implementation cannot change without a specification/ADR update. SHOULD/SHOULD NOT define strong recommendations that may be overridden only with a documented reason.

### State and evidence rule
A user-visible success message, transport callback, database row, or ML result is not independently authoritative. The authoritative meaning of an event comes from the protocol and state machine governing it. Any implementation that shortcuts the defined verification path is a protocol defect even when it makes a demo appear more reliable.

### Failure-first implementation
For every happy-path step, design its failure path before coding it. Ask: what if the process dies immediately before/after this write? What if the message arrives twice? What if the bytes are modified? What if the device clock is wrong? What if the backend response is lost after acceptance? What if a credential is revoked while the device is disconnected? The answer must be represented in state, error semantics, or an explicit documented residual risk.

### Reproducibility
Security-sensitive and research-sensitive behavior must be deterministic where practical. Test vectors, configuration, protocol version, database schema version, model version, and simulator seed are part of the evidence. A screenshot is not sufficient evidence for a security property.

### Change-control trigger
A change to a field, state, key lifecycle, counter rule, offline budget, transport meaning, reconciliation rule, risk boundary, or trust assumption MUST be treated as a specification change and reviewed through `.agent/CHANGE_CONTROL.md`.
