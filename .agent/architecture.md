...
Source document retained and reorganized from: .agent/architecture.md
...
- UI → Domain only.
- Domain → Crypto Adapter / Ledger / Transport abstractions.
- Transport → Protocol message only.
- Backend verifies evidence independently.
- Risk Engine returns assessment, not cryptographic authorization.

## Dependency rule

Lower-level modules must not depend on higher-level UI concerns. Security-critical modules must have minimal dependencies.
